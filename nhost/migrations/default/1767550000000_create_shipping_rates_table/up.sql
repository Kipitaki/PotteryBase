...
CREATE TABLE IF NOT EXISTS potterbase.shipping_rates (
...
-- Add comment
COMMENT ON TABLE potterbase.shipping_rates IS 'Custom shipping rate estimates by zone and weight range';
COMMENT ON COLUMN potterbase.shipping_rates.zone IS 'USPS zone (1-9)';
COMMENT ON COLUMN potterbase.shipping_rates.weight_min IS 'Minimum weight in ounces (inclusive)';
COMMENT ON COLUMN potterbase.shipping_rates.weight_max IS 'Maximum weight in ounces (exclusive)';
COMMENT ON COLUMN potterbase.shipping_rates.cost IS 'Shipping cost in USD';

-- Create index for faster lookups
CREATE INDEX IF NOT EXISTS idx_shipping_rates_zone_weight ON potterbase.shipping_rates(zone, weight_min, weight_max);
